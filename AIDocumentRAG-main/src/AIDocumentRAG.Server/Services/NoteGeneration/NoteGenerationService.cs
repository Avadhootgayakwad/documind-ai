namespace AIDocumentRAG.Server.Services.NoteGeneration
{
    ...
    public class NoteGenerationService : INoteGenerationService
    {
        ...
        public NoteGenerationService(INoteRepository noteRepository, IDocumentSummaryService documentSummaryService, IAIChatService aiChatService)
        {
        ...
        public async Task<NoteGenerationResponse> GenerateNoteAsync(GenerateNoteRequest request)
        {
            try
            {
                ...
                string generatedContent = await GenerateNoteContent(prompt);
        ...
        private async Task<string> GenerateNoteContent(string prompt)
        {
            try
            {
                // Use the AI chat service to generate note content
                return await _aiChatService.GenerateResponseAsync(prompt, "ollama", "phi4");
            }
            catch
            {
                // Fallback to basic note structure if AI service fails
