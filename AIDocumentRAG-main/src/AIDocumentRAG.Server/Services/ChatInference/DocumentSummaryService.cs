namespace AIDocumentRAG.Server.Services.ChatInference
{
    ...
    public class DocumentSummaryService : IDocumentSummaryService
    {
        ...
        public DocumentSummaryService(IAIChatService aiChatService, ILogger<DocumentSummaryService> logger)
        {
        ...
        public async Task<string> GenerateSummaryAsync(string content, string fileName)
        {
            try
            {
                ...
                // Use the AI chat service to generate summary
                string summary = await this._aiChatService.GenerateResponseAsync(prompt, "ollama", "phi4");
