import { Component, Input, OnChanges, SimpleChanges, ViewChild, ElementRef, AfterViewChecked, OnDestroy, ChangeDetectorRef } from '@angular/core';
...
export class ChatViewComponent implements OnChanges, AfterViewChecked, OnDestroy {
  ...
  private scrollToBottom(): void {
    try {
      if (this.messagesContainer?.nativeElement) {
        ...
        console.log('Scrolling to bottom. scrollHeight:', container.scrollHeight, 'scrollTop:', container.scrollTop);
        ...
        .subscribe({
          next: (response) => {
            if (response.success && response.data?.response) {
              ...
              console.log('Is HTML?', this.isHtmlContent(response.data.response));
              console.log('First 200 chars:', response.data.response.substring(0, 200));
              ...
              console.log('Adding message to array, total messages:', this.messages.length + 1);
              ...
              setTimeout(() => {
                this.triggerScrollToBottom();
                console.log('Scroll triggered after AI response');
              }, 100);
            } else {
  ...
  sanitizeHtml(html: string): SafeHtml {
    // Trust the HTML content from our backend (it's safe since we control it)
